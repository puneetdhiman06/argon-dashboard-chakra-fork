  ...
  const GeneralForm = ({ handleSubmit, sendValidation, initialFormData, photoUpload=true, children }) => {
    ...
    const handleSearch = async () => {
      setLoading(true);
      try {
        const response = await fetch('https://vmware.0.staging.xpponet.in/api/xpponetwork/onu_box_search/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            host: formData.ipAddress, // Use formData or default
            serial_no: formData.serialNumber, // Use formData or default
          }),
        });
    ...
    const fetchEmployeeData = async () => {
      try {
        ...
        const response = await fetch('https://vmware.0.staging.xpponet.in/api/franchiseuser/franchise-employee/', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
        ...
        const names = data.map((item) => item.name || 'N/A');
        const phones = data.map((item) => item.phone_no || 'N/A');

        setEmployeeNames(names);
        setEmployeePhones(phones);
            ...
            <>
              ...
              <FormControl>
                ...
                <Select
                  ...
                  value={formData.engineerName || (employeeNames[0] || '')} // Select 0th index if formData is empty
                  onChange={handleChange}
                  isDisabled={loading}
                >
              ...
              <FormControl>
                ...
                <Select
                  name="engineerPhone"
                  placeholder="Select Engineer Phone"
                  value={formData.engineerPhone || (employeePhones[0] || '')} // Select 0th index if formData is empty
                  onChange={handleChange}
                  isDisabled={loading}
                >
                  {employeePhones.map((phone, index) => (
